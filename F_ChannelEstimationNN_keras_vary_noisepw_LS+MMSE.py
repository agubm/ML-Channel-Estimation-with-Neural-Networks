...
#Channel Estimation/Prediction with Least Square (4-layer RNeural Network, etc)
import numpy as np
...
accuracy = [] #store the prediction accuracy per loop (for diff. antenna array sizes). We can visualize how the accuracy decrease with high noise power, but constant epoch value and other parameters.
overall_MSE_NN_all = [] #this store all the mse per noise power. I can then find a what to extract them, and plot all the values on a single graph
overall_MSE_LS_all = [] #same here as above.
...
epoch = 194
ite = 10000
noisepwr = list(range(1,8,2)) #here, I generated the noise power levels.
...
noise = np.random.randn(ite,nr,1) #the noise coeffs. has to be constant. What changes will be the power
training = 6 #training sequence
layer1node = 25 # number of nodes in first layer
layer2anode = 25 # number of nodes in second layer (hidden)
layer2bnode = 25 # number of nodes in third layer (hidden)
...
def ChannelEstimationLS(jj):
    global y_all_nnn, ccchannel,stepSize, ite_t, summary, ite, idx, Chanel_MMSE_v, Chanel_MMSE_t, Chanel_v, h, Chanel_t, batch_size, Chanel_test, Noise, x, y_all_N
    ...
    MSE_n_all = [] # stores the MSE values for coeff. Rn estimation
    ...
    # Channel (idealized without nosie or true channel coefficients 
        for i in range (ite):
            Chanel = np.random.randn(nr,nt)# same channel for varying noise and constant noise power. Recall: Its LTI
            y = np.add(np.dot(Chanel,x),(jj*noise[i]))
...
#this is the main activator:
for jj in range(len(noisepwr)):
    print ('LS Channel Estimation for noise power of', noisepwr[jj])
    ChannelEstimationLS(jj)
    print ('MMSE Channel Estimation for noise power of', noisepwr[jj])
    ChannelEstimationMMSE() #this will not be a function of jj anymore. I am reusing the data i got from the LS computations.
    
#make a simply plot of the varying accuracy as noise power is introduced.  
plt.plot(accuracy, '-*')
plt.title('Graph of LS prediction accuracy with varying noise power (before training)')
plt.ylabel('Predicition Accuracy')
plt.xlabel('Varying Noise Power')
plt.grid(b=None, which='major', axis='both')
plt.show()
  
plt.plot(accuracy_MMSE, '-*')
plt.title('Graph of MMSE prediction accuracy with varying noise power (before training)')
plt.ylabel('Predicition Accuracy')
plt.xlabel('Varying Noise Power')
plt.grid(b=None, which='major', axis='both')
plt.show()