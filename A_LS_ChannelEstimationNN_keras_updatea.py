...
epoch = 195
ite = 2000                                #This determines the 3rd dimension of the Tensor. With this, we can have: 40 by 4 by 4 Tensor (ie. if ite = 40)
...
ite_t = 500 #the number of test channel for which i will compute the average mse
ccchannel = np.random.randn(ite_t,nr,nt)
y_all_nnn = [] 
Channel_LS_all_nn = []
Channel_pred_all_nn = []

#generate SNR conmensurate with the channels i have in order to correct the plot: #Evaluting performance with varying mse vs snr:  #Obtained a vector with varying noise power
start_ = 15
stop_ = 0.01
...
for eee in range(len(ccchannel)):
    hNN_pred_ = np.reshape(Channel_NN_all_nnA[eee+aa],(-1)) #reshapes or flattens the vector to allow being used for plotting
    hLS_ = np.reshape(Channel_LS_all_nnA[eee+aa],(-1))
    h_ = np.reshape(ccchannel[eee], (-1))
    MSE1_ = np.mean((h_ - hNN_pred_)**2) #to obtain the MSE = (mean(pow(hLS - h), 2))
